...
const OeProformaPage: React.FC = () => {
  ...
  const handleSelect = async (name: string) => {
    ...
    try {
      ...
      if (productPrice <= 0) {
        alert("Pa gen pri pou item sa. Mete price avan.");
        setPrice(0);
        return;
      }
  ...
  const addItem = () => {
    ...
    const selectedProduct = stocks.find((s: any) => s.itemName === selectedItem);
    ...
    const alreadyExists = items.find((i) => i.productId === selectedProduct.id);
  ...
  const buildPDF = (proforma: any) => {
    const doc = new jsPDF();
    let y = 15;

    doc.setFont("helvetica", "bold");
    doc.setFontSize(18);
    doc.text(company?.companyName || "", 105, y, { align: "center" });
    ...
    const barcode = generateBarcode(proforma.proformaNumber);
    doc.addImage(barcode, "PNG", 135, y - 8, 60, 20);

    doc.setFont("helvetica", "normal");
    doc.setFontSize(12);

    doc.text(`Proforma: ${proforma.proformaNumber}`, 20, y);
    y += 8;

    doc.text(`Customer: ${proforma.customerName}`, 20, y);
    y += 8;

    doc.text(`Created By: ${proforma.cashier}`, 20, y);
    y += 8;
    ...
    doc.text(money(proforma.subtotal), 190, y, { align: "right" });
    y += 8;

    doc.text(`Tax (${proforma.taxRate}%):`, 120, y);
    doc.text(money(proforma.taxAmount), 190, y, { align: "right" });
    y += 8;

    doc.setFont("helvetica", "bold");
    doc.text("TOTAL:", 120, y);
    doc.text(money(proforma.total), 190, y, { align: "right" });

    y += 18;

    doc.setFont("helvetica", "italic");
    doc.setFontSize(10);
    doc.text("This is a proforma invoice. It is not a final sale receipt.", 105, y, {
      align: "center",
    });
  ...
  const createProforma = () => {
    ...
    if (grandTotal <= 0) {
      alert("Proforma total must be greater than 0");
      return;
    }
    ...
    const proformaData = {
      ...
      taxAmount,
      total: grandTotal,
      barcode: generateBarcode(proformaNumber),
    };

    buildPDF(proformaData);
    setReceipt(proformaData);
    ...
    <div style={styles.page}>
      <div style={styles.card}>
        <div style={styles.header}>
          <div>
            <h1 style={styles.title}>New Proforma</h1>
            <p style={styles.subtitle}>Create proforma invoice</p>
          </div>

          <button onClick={newProforma} style={styles.secondaryBtn}>
            New Proforma
          </button>
        </div>
        ...
        <div style={styles.footer}>
          <div style={styles.totals}>
            ...
            <p>
              Tax ({taxRate}%): <b>{money(taxAmount)}</b>
            </p>
            <h2>Total: {money(grandTotal)}</h2>
          </div>

          <button onClick={createProforma} style={styles.submitBtn}>
            Create Proforma
          </button>
        </div>
        ...
        {pdfUrl && (
          <div className="no-print" style={{ marginTop: 20 }}>
            <a href={pdfUrl} download="proforma.pdf">
          ...
          <>
            ...
            <div className="receipt">
              <div className="receipt-center">
                <div className="receipt-title">{company?.companyName || "STORE"}</div>
                <div className="receipt-subtitle">{company?.address || ""}</div>
                <div className="receipt-subtitle">{company?.phone || ""}</div>

                {receipt.barcode && (
                  <img src={receipt.barcode} alt="barcode" className="receipt-barcode" />
                )}
              </div>
              ...
              <div className="receipt-row">
                <span>Proforma</span>
                <span>{receipt.proformaNumber}</span>
              </div>
              ...
              {receipt.items.map((item: any, i: number) => (
                <div key={i}>
                  <div className="receipt-item-name">{item.itemName}</div>
              ...
              <div className="receipt-footer">
                PROFORMA ONLY - NOT A FINAL SALE
              </div>
            </div>
          </>
        )}
