...
const ARAgingReportPage: React.FC = () => {
  ...
  const loadARAging = async () => {
    try {
      const res = await axios.get(`${API}/ar/aging`);
      setRows(res.data?.data || []);
    } catch (err) {
      console.log("AR AGING LOAD ERROR 👉", err);
      setRows([]);
    }
  };
    ...
    <div style={styles.page}>
      <div style={styles.card}>
        <div style={styles.header}>
          <div>
            <h1 style={styles.title}>AR Aging Report</h1>
            <p style={styles.subtitle}>Customer receivable aging balances</p>
          </div>

          <button onClick={loadARAging} style={styles.secondaryBtn}>
            Refresh
          </button>
        </div>
        ...
        <div style={styles.tableWrap}>
          <table style={styles.table}>
            ...
            <tbody>
              {filteredRows.length === 0 ? (
                ...
                filteredRows.map((row, index) => (
                  <tr key={row.id || index}>
                    ...
                    <td style={styles.td}>
                      {String(row.invoiceDate || "").split("T")[0]}
                    </td>
              ...
              {filteredRows.length > 0 && (
                <tr>
                  <td style={styles.totalTd} colSpan={4}>
                    TOTAL
                  </td>
